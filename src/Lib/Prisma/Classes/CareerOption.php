...
class CareerOption implements IModel
{
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->_fieldsOnly = ['id', 'code', 'career', 'shift', 'level', 'area', 'description', 'createdAt', 'updatedAt'];
        $this->_tableFieldsOnly = ['id', 'code', 'career', 'shift', 'level', 'area', 'description', 'createdAt', 'updatedAt'];
        $this->_fieldsRelated = [];
        ...
        $this->_primaryKeyAndUniqueFields = ['id', 'code'];
        $this->_fieldsCombined = ['id', 'code', 'career', 'shift', 'level', 'area', 'description', 'createdAt', 'updatedAt'];

        $this->_pdo = $pdo;
        $this->_dbType = $this->_pdo->getAttribute(PDO::ATTR_DRIVER_NAME);
        $this->_modelName = 'CareerOption';
        $this->_tableName = 'CareerOption';
     ...
     *
     * This method allows dynamic access to properties of the CareerOption class.
    ...
    public function __get(string $name)
    {
        ...
        throw new Exception("Property '$name' does not exist in CareerOption.");
    }
    ...
    private function buildOneToManyRelationClause(
        ...
        $fromField = PPHPUtility::quoteColumnName(
            $this->_dbType,
            $relatedFieldKeys['relationFromFields'][0]
        );
        $toField = PPHPUtility::quoteColumnName(
            $this->_dbType,
            $relatedFieldKeys['relationToFields'][0]
        );
        ...
        switch ($conditionType) {
            case 'some':
                return "EXISTS(SELECT 1 FROM $relatedTable AS j0 " .
                    ...
                    "AND j0.$toField = $quotedTableName.$fromField)";
    ...
    /**
     * Performs an aggregate operation on the 'CareerOption' table. This method allows for flexible aggregation queries
     ...
     *
     * @return CareerOptionData Returns an object containing the results of the aggregate operation. The result will include keys corresponding to the specified
     *
     ...
     *     'where' => ['status' => 'active'],
     * ];
     * $result = $prisma->careerOption->aggregate($criteria);
     ...
     * queries with conditions (WHERE clause), ordering (ORDER BY), and pagination (LIMIT and OFFSET) on the 'CareerOption' table.
    ...
    /**
     * Creates a new CareerOption in the database.
     *
     * This method is designed to insert a new CareerOption record into the database using provided data.
     * It is capable of handling related records through the dynamically defined relations in the CareerOption model.
     ...
     *
     * @return CareerOptionData The newly created CareerOptionData record.
     *
     * @throws:
     ...
     * Example:
     * ```
     * Example of creating a new CareerOption with related profile and roles
     * $result = $prisma->careerOption->create([
     *   'data' => [
     *     'property' => 'value',
     * ]);
     * ```
    ...
    /**
     * Bulk Create CareerOption – Fast, Safe, and Flexible
     *
     * Effortlessly add multiple CareerOption records to your database in a single, atomic operation.
     * This method is perfect for onboarding, migrations, or any scenario where you need to insert many CareerOption records at once.
     *
     * Key Features:
     * - Accepts an array of CareerOption records—add as many as you need in one call.
     * - Runs inside a transaction: if any record fails, nothing is saved.
     * - Validates required fields for every CareerOption before saving.
     ...
     *   ];
     *   $result = $prisma->careerOption->createMany([
     *     'data' => $data,
     ...
     *
     * @param:
     * - array $data: Must include a 'data' key with an array of CareerOption records.
     * - skipDuplicates (optional): Set to true to ignore duplicates if supported by your DB.
     *
     * @return:
     * - An object like (object)['count' => N], where N is the number of CareerOptions created.
     *
     * @throws:
    ...
    /**
     * Bulk Create CareerOption and Return Created Records
     *
     * This method allows you to create multiple CareerOption records in a single operation and returns the created records.
     * It supports skipping duplicates and allows for selective field retrieval.
     *
     * @param array $data An associative array containing:
     * - 'data': A list of associative arrays representing the CareerOption records to create.
     ...
     *
     * @return array An array of created CareerOptionData objects.
     *
    ...
    /**
     * Retrieves a single CareerOption record matching specified criteria.
     * 
     * Searches for a unique CareerOption based on the provided filter criteria within `$criteria`.
     * It returns the CareerOption data as either an associative array or an object, based on the `$format` parameter.
     * This method supports filtering (`where`), field selection (`select`), and inclusion of related models (`include`).
     * If no matching CareerOption is found, an empty array is returned. The method includes comprehensive error handling for invalid inputs and parameter conflicts.
     *
     * @param array $criteria Filter criteria with keys:
     *  - 'where': Conditions to filter CareerOption records.
     *  - 'select': Fields of the CareerOption to return.
     *  - 'include': Related models to include in the result.
     * @return CareerOptionData|null The CareerOptionData record matching the criteria.
     * 
     ...
     * 
     * @example
     * To find a CareerOption by ID, select specific fields, and include related models:
     * $result = $prisma->careerOption->findUnique([
     *   'where' => ['id' => 'someUserId'],
    ...
    public function findUnique(array $criteria): ?object
    {
        ...
        if (empty($conditions)) {
            throw new Exception("No valid 'where' conditions provided for finding a unique record in CareerOption.");
        }
    ...
    /**
     * Retrieves a unique CareerOption record based on specified criteria.
     *
     * This method is a wrapper around `findUnique` that throws an exception if no record is found.
     * It is useful when you expect exactly one CareerOption to match the criteria and want to handle the case where none exists.
     *
     * @param array $criteria Criteria to find the unique CareerOption, same as in `findUnique`.
     * @return CareerOptionData The CareerOptionData record matching the criteria.
     * @throws Exception If no CareerOption record matches the criteria.
     */
    ...
    /**
     * Retrieves multiple CareerOption records based on specified filter criteria.
     *
     ...
     * if no CareerOptions match the criteria. This approach ensures flexibility and efficiency in fetching data
     ...
     * @return CareerOptionData[] An array of CareerOptionData objects or an empty array
     * 
     * @example
     * Retrieve CareerOption with cursor-based pagination:
     * $result = $prisma->careerOption->findMany([
     *   'cursor' => ['id' => 'someUserId'],
     *   'take' => 5
     * ]);
     * 
     * Select specific fields of CareerOption:
     * $result = $prisma->careerOption->findMany([
    ...
    /**
     * Retrieves the first CareerOption record that matches specified filter criteria.
     *
     * Designed to efficiently find and return the first CareerOption record matching the provided criteria.
     ...
     *   - 'where': Filter criteria for searching CareerOption records.
     ...
     *
     * Examples:
     * Find a CareerOption by email, returning specific fields:
     * $result = $prisma->careerOption->findFirst([
     *   'where' => ['email' => 'user@example.com'],
     *   'select' => ['id', 'email', 'name']
     * ]);
     * Find an active CareerOption, include their posts, ordered by name:
     * $result = $prisma->careerOption->findFirst([
     *   'where' => ['active' => true],
     *   'orderBy' => 'name',
     *   'include' => ['posts' => true]
    ...
    /**
     * Retrieves the first CareerOption record that matches specified filter criteria or throws an exception if not found.
     *
     * This method is similar to `findFirst`, but it guarantees that a CareerOption record is returned. If no record
     ...
     * 
     * Returns:
     * @return CareerOptionData|array|null The updated User record or an empty array if no match is found.
     * 
     * Example Usage:
     * Example 1: Update a CareerOption's email and only return their 'id' and 'email' in the response
     * $result = $prisma->careerOption->update([
     *   'where' => ['id' => 'someUserId'],
     *   'data' => ['email' => 'new.email@example.com'],
     *   'select' => ['id' => true, 'email' => true]
     * ]);
     * 
     * Example 2: Update a CareerOption's username and include their profile information in the response
     * $result = $prisma->careerOption->update([
     *   'where' => ['id' => 'someUserId'],
    ...
    {
        if (!isset($data['where'])) {
            throw new Exception("The 'where' key is required in the update CareerOption.");
        }
        ...
        if (!isset($data['data'])) {
            throw new Exception("The 'data' key is required in the update CareerOption.");
        }
    ...
    /**
     * Bulk update CareerOption records with flexible conditions and smart validation.
     *
     * This method lets you update multiple CareerOption records at once, based on any conditions you specify.
     ...
     *
     * Example:
     *   $result = $prisma->careerOption->updateMany([
     *     'where' => ['name' => null],
     *     'data'  => ['name' => 'Anonymous']
     *   ]);
     *   $result = (object)['count' => 3]
     *
     * This is perfect for mass updates—like activating CareerOption, anonymizing data, or fixing typos—quickly and safely.
     */
    ...
    /**
     * Updates multiple CareerOption records and returns the updated records.
     *
     * This method allows you to update multiple CareerOption records based on specified conditions
     ...
     *
     * @return array An array of updated CareerOptionData objects.
     *
     ...
     * @return CareerOptionData|array On successful deletion, returns the deleted CareerOptionData record.
     *               If the deletion is unsuccessful due to a non-existent CareerOptionData or non-unique criteria, returns an
     *               array with 'modelName' and 'cause' keys, indicating the reason for failure.
     * 
     * @example
     * Delete a CareerOption by ID and return the deleted User's data as an array
     * $result = $prisma->careerOption->delete([
     *   'where' => ['id' => 'someUserId']
     * ]);
     * 
     * @example
     * Delete a CareerOption by ID, selecting specific fields to return
     * $result = $prisma->careerOption->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'select' => ['name' => true, 'email' => true]
     * ]);
     * 
     * @example
     * Delete a CareerOption by ID, including related records in the return value
     * $result = $prisma->careerOption->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'include' => ['posts' => true]
     * ]);
    ...
    public function delete(array $criteria): object|array
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'CareerOption', 'cause' => 'Record to delete does not exist.'];
        } catch (Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    /**
     * Performs an Upsert Operation on a CareerOption Record
     *
     * Implements an "upsert" operation for CareerOption records. This method checks for the existence of a CareerOption based on
     * specified 'where' conditions. If the CareerOption exists, it updates the CareerOption with the provided 'update' data;
     * if the CareerOption does not exist, it creates a new CareerOption with the 'create' data. This operation is atomic, ensuring
     ...
     * 
     * @return CareerOptionData Returns the created or updated User record.
     * 
     * Exceptions:
     ...
     * 
     * Example Usage:
     * $result = $prisma->careerOption->upsert([
     *   'where' => ['email' => 'user@example.com'],
    ...
    /**
     * Groups records in the 'CareerOption' table based on specified criteria.
     * 
     * This function groups records in the 'CareerOption' table according to the fields specified 
    ...
    /**
     * Dynamically deletes multiple CareerOption records based on flexible criteria.
     *
     * This method enables bulk deletion of CareerOption records by specifying any combination of conditions in the 'where' key.
     * - Accepts any valid CareerOption field(s) as filter criteria.
     ...
     *
     * Example:
     *   $result = $prisma->careerOption->deleteMany([
     ...
     * @return int|array|CareerOptionData The number of records that match the criteria or detailed count per field.
     * @throws Exception Throws an exception if the database operation fails.
     *
     * Example Usage:
     * $prisma = new Prisma();
     * $countCriteria = ['status' => 'active'];
     * $result = $prisma->careerOption->count($countCriteria);
     */
    public function count(array $criteria = []): int|array|object
    {
        ...
        $out = [];
        foreach (array_keys($select) as $field) {
            $out[$field] = (int)($row['count_' . $field] ?? 0);
        }
        return (object)$out;
    }
}
