...
class Chat implements IModel
{
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->_fieldsOnly = ['id', 'title', 'createdAt', 'updatedAt', 'userId'];
        $this->_tableFieldsOnly = ['id', 'title', 'createdAt', 'updatedAt', 'userId'];
        $this->_fieldsRelated = ['user', 'messages'];
        ...
        $this->_fieldsCombined = ['id', 'title', 'createdAt', 'updatedAt', 'userId', 'user', 'messages'];

        $this->_pdo = $pdo;
        $this->_dbType = $this->_pdo->getAttribute(PDO::ATTR_DRIVER_NAME);
        $this->_modelName = 'Chat';
        $this->_tableName = 'Chat';
     ...
     *
     * This method allows dynamic access to properties of the Chat class.
    ...
    public function __get(string $name)
    {
        ...
        throw new Exception("Property '$name' does not exist in Chat.");
    }
    ...
    /**
     * Performs an aggregate operation on the 'Chat' table. This method allows for flexible aggregation queries
     ...
     *
     * @return ChatData Returns an object containing the results of the aggregate operation. The result will include keys corresponding to the specified
     *
     ...
     *     'where' => ['status' => 'active'],
     * ];
     * $result = $prisma->chat->aggregate($criteria);
     ...
     * queries with conditions (WHERE clause), ordering (ORDER BY), and pagination (LIMIT and OFFSET) on the 'Chat' table.
     ...
     *
     * This method is designed to insert a new Chat record into the database using provided data.
     * It is capable of handling related records through the dynamically defined relations in the Chat model.
     ...
     *
     * @return ChatData The newly created ChatData record.
     *
     * @throws:
     ...
     * Example:
     * ```
     * Example of creating a new Chat with related profile and roles
     * $result = $prisma->chat->create([
     *   'data' => [
     *     'property' => 'value',
     * ]);
     * ```
    ...
    /**
     * Bulk Create Chat – Fast, Safe, and Flexible
     *
     * Effortlessly add multiple Chat records to your database in a single, atomic operation.
     * This method is perfect for onboarding, migrations, or any scenario where you need to insert many Chat records at once.
     *
     * Key Features:
     * - Accepts an array of Chat records—add as many as you need in one call.
     * - Runs inside a transaction: if any record fails, nothing is saved.
     * - Validates required fields for every Chat before saving.
     ...
     *
     * @param:
     * - array $data: Must include a 'data' key with an array of Chat records.
     * - skipDuplicates (optional): Set to true to ignore duplicates if supported by your DB.
     *
     * @return:
     * - An object like (object)['count' => N], where N is the number of Chats created.
     *
     * @throws:
    ...
    /**
     * Bulk Create Chat and Return Created Records
     *
     * This method allows you to create multiple Chat records in a single operation and returns the created records.
     * It supports skipping duplicates and allows for selective field retrieval.
     *
     * @param array $data An associative array containing:
     * - 'data': A list of associative arrays representing the Chat records to create.
     ...
     *
     * @return array An array of created ChatData objects.
     *
    ...
    /**
     * Retrieves a single Chat record matching specified criteria.
     * 
     * Searches for a unique Chat based on the provided filter criteria within `$criteria`.
     * It returns the Chat data as either an associative array or an object, based on the `$format` parameter.
     * This method supports filtering (`where`), field selection (`select`), and inclusion of related models (`include`).
     * If no matching Chat is found, an empty array is returned. The method includes comprehensive error handling for invalid inputs and parameter conflicts.
     *
     * @param array $criteria Filter criteria with keys:
     *  - 'where': Conditions to filter Chat records.
     *  - 'select': Fields of the Chat to return.
     *  - 'include': Related models to include in the result.
     * @return ChatData|null The ChatData record matching the criteria.
     * 
     ...
     * 
     * @example
     * To find a Chat by ID, select specific fields, and include related models:
     * $result = $prisma->chat->findUnique([
     *   'where' => ['id' => 'someUserId'],
    ...
    public function findUnique(array $criteria): ?object
    {
        ...
        if (empty($conditions)) {
            throw new Exception("No valid 'where' conditions provided for finding a unique record in Chat.");
        }
    ...
    /**
     * Retrieves a unique Chat record based on specified criteria.
     *
     * This method is a wrapper around `findUnique` that throws an exception if no record is found.
     * It is useful when you expect exactly one Chat to match the criteria and want to handle the case where none exists.
     *
     * @param array $criteria Criteria to find the unique Chat, same as in `findUnique`.
     * @return ChatData The ChatData record matching the criteria.
     * @throws Exception If no Chat record matches the criteria.
     */
    ...
    /**
     * Retrieves multiple Chat records based on specified filter criteria.
     *
     ...
     * if no Chats match the criteria. This approach ensures flexibility and efficiency in fetching data
     ...
     * @return ChatData[] An array of ChatData objects or an empty array
     * 
     * @example
     * Retrieve Chat with cursor-based pagination:
     * $result = $prisma->chat->findMany([
     *   'cursor' => ['id' => 'someUserId'],
     *   'take' => 5
     * ]);
     * 
     * Select specific fields of Chat:
     * $result = $prisma->chat->findMany([
    ...
    /**
     * Retrieves the first Chat record that matches specified filter criteria.
     *
     * Designed to efficiently find and return the first Chat record matching the provided criteria.
     ...
     *   - 'where': Filter criteria for searching Chat records.
     ...
     *
     * Examples:
     * Find a Chat by email, returning specific fields:
     * $result = $prisma->chat->findFirst([
     *   'where' => ['email' => 'user@example.com'],
     *   'select' => ['id', 'email', 'name']
     * ]);
     * Find an active Chat, include their posts, ordered by name:
     * $result = $prisma->chat->findFirst([
     *   'where' => ['active' => true],
     *   'orderBy' => 'name',
     *   'include' => ['posts' => true]
    ...
    /**
     * Retrieves the first Chat record that matches specified filter criteria or throws an exception if not found.
     *
     * This method is similar to `findFirst`, but it guarantees that a Chat record is returned. If no record
     ...
     * 
     * Returns:
     * @return ChatData|array|null The updated User record or an empty array if no match is found.
     * 
     * Example Usage:
     * Example 1: Update a Chat's email and only return their 'id' and 'email' in the response
     * $result = $prisma->chat->update([
     *   'where' => ['id' => 'someUserId'],
     *   'data' => ['email' => 'new.email@example.com'],
     *   'select' => ['id' => true, 'email' => true]
     * ]);
     * 
     * Example 2: Update a Chat's username and include their profile information in the response
     * $result = $prisma->chat->update([
     *   'where' => ['id' => 'someUserId'],
    ...
    {
        if (!isset($data['where'])) {
            throw new Exception("The 'where' key is required in the update Chat.");
        }
        ...
        if (!isset($data['data'])) {
            throw new Exception("The 'data' key is required in the update Chat.");
        }
    ...
    /**
     * Bulk update Chat records with flexible conditions and smart validation.
     *
     * This method lets you update multiple Chat records at once, based on any conditions you specify.
     ...
     *
     * Example:
     *   $result = $prisma->chat->updateMany([
     *     'where' => ['name' => null],
     *     'data'  => ['name' => 'Anonymous']
     *   ]);
     *   $result = (object)['count' => 3]
     *
     * This is perfect for mass updates—like activating Chat, anonymizing data, or fixing typos—quickly and safely.
     */
    ...
    /**
     * Updates multiple Chat records and returns the updated records.
     *
     * This method allows you to update multiple Chat records based on specified conditions
     ...
     *
     * @return array An array of updated ChatData objects.
     *
     ...
     * @return ChatData|array On successful deletion, returns the deleted ChatData record.
     *               If the deletion is unsuccessful due to a non-existent ChatData or non-unique criteria, returns an
     *               array with 'modelName' and 'cause' keys, indicating the reason for failure.
     * 
     * @example
     * Delete a Chat by ID and return the deleted User's data as an array
     * $result = $prisma->chat->delete([
     *   'where' => ['id' => 'someUserId']
     * ]);
     * 
     * @example
     * Delete a Chat by ID, selecting specific fields to return
     * $result = $prisma->chat->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'select' => ['name' => true, 'email' => true]
     * ]);
     * 
     * @example
     * Delete a Chat by ID, including related records in the return value
     * $result = $prisma->chat->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'include' => ['posts' => true]
     * ]);
    ...
    public function delete(array $criteria): object|array
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'Chat', 'cause' => 'Record to delete does not exist.'];
        } catch (Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    /**
     * Performs an Upsert Operation on a Chat Record
     *
     * Implements an "upsert" operation for Chat records. This method checks for the existence of a Chat based on
     * specified 'where' conditions. If the Chat exists, it updates the Chat with the provided 'update' data;
     * if the Chat does not exist, it creates a new Chat with the 'create' data. This operation is atomic, ensuring
     ...
     * 
     * @return ChatData Returns the created or updated User record.
     * 
     * Exceptions:
    ...
    /**
     * Groups records in the 'Chat' table based on specified criteria.
     * 
     * This function groups records in the 'Chat' table according to the fields specified 
    ...
    /**
     * Dynamically deletes multiple Chat records based on flexible criteria.
     *
     * This method enables bulk deletion of Chat records by specifying any combination of conditions in the 'where' key.
     * - Accepts any valid Chat field(s) as filter criteria.
     ...
     * @return int|array|ChatData The number of records that match the criteria or detailed count per field.
     * @throws Exception Throws an exception if the database operation fails.
     *
     * Example Usage:
     * $prisma = new Prisma();
     * $countCriteria = ['status' => 'active'];
     * $result = $prisma->chat->count($countCriteria);
     */
