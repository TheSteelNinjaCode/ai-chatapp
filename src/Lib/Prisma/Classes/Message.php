...
class Message implements IModel
{
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->_fieldsOnly = ['id', 'content', 'role', 'createdAt', 'promptTokens', 'completionTokens', 'chatId'];
        $this->_tableFieldsOnly = ['id', 'content', 'role', 'createdAt', 'promptTokens', 'completionTokens', 'chatId'];
        $this->_fieldsRelated = ['chat'];
        $this->_fieldsRelatedWithKeys = [
            'chat' => [
                "relationFromFields" => ['chatId'],
                "relationToFields" => ['id']
            ]
        ];
        $this->_primaryKey = 'id';
        ...
        $this->_fieldsCombined = ['id', 'content', 'role', 'createdAt', 'promptTokens', 'completionTokens', 'chatId', 'chat'];

        $this->_pdo = $pdo;
        $this->_dbType = $this->_pdo->getAttribute(PDO::ATTR_DRIVER_NAME);
        $this->_modelName = 'Message';
     ...
     *
     * This method allows dynamic access to properties of the Message class.
    ...
    public function __get(string $name)
    {
        ...
        throw new Exception("Property '$name' does not exist in Message.");
    }
    ...
    /**
     * Performs an aggregate operation on the 'Message' table. This method allows for flexible aggregation queries
     ...
     *
     * @return MessageData Returns an object containing the results of the aggregate operation. The result will include keys corresponding to the specified
     *
     ...
     *     'where' => ['status' => 'active'],
     * ];
     * $result = $prisma->message->aggregate($criteria);
     ...
     * queries with conditions (WHERE clause), ordering (ORDER BY), and pagination (LIMIT and OFFSET) on the 'Message' table.
     ...
     *
     * This method is designed to insert a new Message record into the database using provided data.
     * It is capable of handling related records through the dynamically defined relations in the Message model.
     ...
     *
     * @return MessageData The newly created MessageData record.
     *
     * @throws:
     ...
     * Example:
     * ```
     * Example of creating a new Message with related profile and roles
     * $result = $prisma->message->create([
     *   'data' => [
     *     'property' => 'value',
     * ]);
     * ```
    ...
    /**
     * Bulk Create Message – Fast, Safe, and Flexible
     *
     * Effortlessly add multiple Message records to your database in a single, atomic operation.
     * This method is perfect for onboarding, migrations, or any scenario where you need to insert many Message records at once.
     *
     * Key Features:
     * - Accepts an array of Message records—add as many as you need in one call.
     * - Runs inside a transaction: if any record fails, nothing is saved.
     * - Validates required fields for every Message before saving.
     ...
     *   ];
     *   $result = $prisma->message->createMany([
     *     'data' => $data,
     ...
     *
     * @param:
     * - array $data: Must include a 'data' key with an array of Message records.
     * - skipDuplicates (optional): Set to true to ignore duplicates if supported by your DB.
     *
     * @return:
     * - An object like (object)['count' => N], where N is the number of Messages created.
     *
     * @throws:
    ...
    /**
     * Bulk Create Message and Return Created Records
     *
     * This method allows you to create multiple Message records in a single operation and returns the created records.
     * It supports skipping duplicates and allows for selective field retrieval.
     *
     * @param array $data An associative array containing:
     * - 'data': A list of associative arrays representing the Message records to create.
     ...
     *
     * @return array An array of created MessageData objects.
     *
    ...
    /**
     * Retrieves a single Message record matching specified criteria.
     * 
     * Searches for a unique Message based on the provided filter criteria within `$criteria`.
     * It returns the Message data as either an associative array or an object, based on the `$format` parameter.
     * This method supports filtering (`where`), field selection (`select`), and inclusion of related models (`include`).
     * If no matching Message is found, an empty array is returned. The method includes comprehensive error handling for invalid inputs and parameter conflicts.
     *
     * @param array $criteria Filter criteria with keys:
     *  - 'where': Conditions to filter Message records.
     *  - 'select': Fields of the Message to return.
     *  - 'include': Related models to include in the result.
     * @return MessageData|null The MessageData record matching the criteria.
     * 
     ...
     * 
     * @example
     * To find a Message by ID, select specific fields, and include related models:
     * $result = $prisma->message->findUnique([
     *   'where' => ['id' => 'someUserId'],
    ...
    public function findUnique(array $criteria): ?object
    {
        ...
        if (empty($conditions)) {
            throw new Exception("No valid 'where' conditions provided for finding a unique record in Message.");
        }
    ...
    /**
     * Retrieves a unique Message record based on specified criteria.
     *
     * This method is a wrapper around `findUnique` that throws an exception if no record is found.
     * It is useful when you expect exactly one Message to match the criteria and want to handle the case where none exists.
     *
     * @param array $criteria Criteria to find the unique Message, same as in `findUnique`.
     * @return MessageData The MessageData record matching the criteria.
     * @throws Exception If no Message record matches the criteria.
     */
    ...
    /**
     * Retrieves multiple Message records based on specified filter criteria.
     *
     ...
     * if no Messages match the criteria. This approach ensures flexibility and efficiency in fetching data
     ...
     * @return MessageData[] An array of MessageData objects or an empty array
     * 
     * @example
     * Retrieve Message with cursor-based pagination:
     * $result = $prisma->message->findMany([
     *   'cursor' => ['id' => 'someUserId'],
     *   'take' => 5
     * ]);
     * 
     * Select specific fields of Message:
     * $result = $prisma->message->findMany([
    ...
    /**
     * Retrieves the first Message record that matches specified filter criteria.
     *
     * Designed to efficiently find and return the first Message record matching the provided criteria.
     ...
     *   - 'where': Filter criteria for searching Message records.
     ...
     *
     * Examples:
     * Find a Message by email, returning specific fields:
     * $result = $prisma->message->findFirst([
     *   'where' => ['email' => 'user@example.com'],
     *   'select' => ['id', 'email', 'name']
     * ]);
     * Find an active Message, include their posts, ordered by name:
     * $result = $prisma->message->findFirst([
     *   'where' => ['active' => true],
     *   'orderBy' => 'name',
     *   'include' => ['posts' => true]
    ...
    /**
     * Retrieves the first Message record that matches specified filter criteria or throws an exception if not found.
     *
     * This method is similar to `findFirst`, but it guarantees that a Message record is returned. If no record
     ...
     * 
     * Returns:
     * @return MessageData|array|null The updated User record or an empty array if no match is found.
     * 
     * Example Usage:
     * Example 1: Update a Message's email and only return their 'id' and 'email' in the response
     * $result = $prisma->message->update([
     *   'where' => ['id' => 'someUserId'],
     *   'data' => ['email' => 'new.email@example.com'],
     *   'select' => ['id' => true, 'email' => true]
     * ]);
     * 
     * Example 2: Update a Message's username and include their profile information in the response
     * $result = $prisma->message->update([
     *   'where' => ['id' => 'someUserId'],
    ...
    {
        if (!isset($data['where'])) {
            throw new Exception("The 'where' key is required in the update Message.");
        }
        ...
        if (!isset($data['data'])) {
            throw new Exception("The 'data' key is required in the update Message.");
        }
    ...
    /**
     * Bulk update Message records with flexible conditions and smart validation.
     *
     * This method lets you update multiple Message records at once, based on any conditions you specify.
     ...
     *
     * Example:
     *   $result = $prisma->message->updateMany([
     *     'where' => ['name' => null],
     *     'data'  => ['name' => 'Anonymous']
     *   ]);
     *   $result = (object)['count' => 3]
     *
     * This is perfect for mass updates—like activating Message, anonymizing data, or fixing typos—quickly and safely.
     */
    ...
    /**
     * Updates multiple Message records and returns the updated records.
     *
     * This method allows you to update multiple Message records based on specified conditions
     ...
     *
     * @return array An array of updated MessageData objects.
     *
     ...
     * @return MessageData|array On successful deletion, returns the deleted MessageData record.
     *               If the deletion is unsuccessful due to a non-existent MessageData or non-unique criteria, returns an
     *               array with 'modelName' and 'cause' keys, indicating the reason for failure.
     * 
     * @example
     * Delete a Message by ID and return the deleted User's data as an array
     * $result = $prisma->message->delete([
     *   'where' => ['id' => 'someUserId']
     * ]);
     * 
     * @example
     * Delete a Message by ID, selecting specific fields to return
     * $result = $prisma->message->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'select' => ['name' => true, 'email' => true]
     * ]);
     * 
     * @example
     * Delete a Message by ID, including related records in the return value
     * $result = $prisma->message->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'include' => ['posts' => true]
     * ]);
    ...
    public function delete(array $criteria): object|array
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'Message', 'cause' => 'Record to delete does not exist.'];
        } catch (Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    /**
     * Performs an Upsert Operation on a Message Record
     *
     * Implements an "upsert" operation for Message records. This method checks for the existence of a Message based on
     * specified 'where' conditions. If the Message exists, it updates the Message with the provided 'update' data;
     * if the Message does not exist, it creates a new Message with the 'create' data. This operation is atomic, ensuring
     ...
     * 
     * @return MessageData Returns the created or updated User record.
     * 
     * Exceptions:
    ...
    /**
     * Groups records in the 'Message' table based on specified criteria.
     * 
     * This function groups records in the 'Message' table according to the fields specified 
    ...
    /**
     * Dynamically deletes multiple Message records based on flexible criteria.
     *
     * This method enables bulk deletion of Message records by specifying any combination of conditions in the 'where' key.
     * - Accepts any valid Message field(s) as filter criteria.
     ...
     *
     * Example:
     *   $result = $prisma->message->deleteMany([
     ...
     * @return int|array|MessageData The number of records that match the criteria or detailed count per field.
     * @throws Exception Throws an exception if the database operation fails.
     *
     * Example Usage:
     * $prisma = new Prisma();
     * $countCriteria = ['status' => 'active'];
     * $result = $prisma->message->count($countCriteria);
     */
