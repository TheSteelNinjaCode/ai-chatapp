...
use Lib\Prisma\Classes\MessageRole;
...
function saveChatTurn($data)
{
    $chatId = Validator::cuid($data->chatId ?? null);
    $title = Validator::string($data->title ?? 'New Chat');
    $userContent = Validator::string($data->userContent ?? '');
    $assistantContent = Validator::string($data->assistantContent ?? '');
    $userId = "cmj7hjop50000mkr3n1wmruzn";
    ...
    if (!$chatId) {
        ...
        $chat = $prisma->chat->create([
            'data' => [
                'title' => $title,
                'userId' => $userId,
                // Create the messages immediately
                'messages' => [
                    'create' => [
                        [
...
<div class="grid h-screen w-full grid-cols-[280px_1fr] grid-rows-[60px_1fr_auto]">
    ...
    <aside class="row-span-3 border-r border-border bg-card hidden md:flex flex-col">
        ...
        <nav class="flex flex-col h-full overflow-hidden p-4 gap-2">

            <div class="flex-none space-y-2 pr-1">
    ...
    <main id="chatScroll" onscroll="handleScroll(event)" class="col-start-2 row-start-2 overflow-y-auto p-4 md:p-10 space-y-8 scroll-smooth">
        ...
        <div class="max-w-3xl mx-auto space-y-8">
            ...
            <template pp-for="m in chat">
                <div class="flex items-start gap-4 {m.role === 'user' ? 'flex-row-reverse' : ''}">
                    ...
                    <div class="h-8 w-8 rounded-full shrink-0 flex items-center justify-center {m.role === 'user' ? 'bg-muted text-muted-foreground font-bold text-xs' : 'bg-primary text-primary-foreground'}">
                        {m.role === 'user' ? 'JA' : ''}
                        <Bot class="size-4" hidden="{m.role === 'user'}" />
                    </div>

                    <div class="space-y-2 w-full max-w-[85%]">
                        <div hidden="{m.role === 'user'}" class="font-semibold text-sm flex items-center gap-2">PulsePoint AI</div>

                        <div class="px-4 py-3 rounded-2xl text-sm shadow-sm 
                {m.role === 'user' 
                    ? 'bg-secondary text-secondary-foreground rounded-tr-sm w-fit ml-auto wrap-break-word' 
                    : 'bg-card text-card-foreground rounded-tl-sm w-fit'}">

                            <span class="whitespace-pre-wrap leading-relaxed" hidden="{m.role === 'assistant' && m.content === ''}">{m.content}</span>

                            <div hidden="{!(m.role === 'assistant' && m.content === '')}" class="flex flex-col gap-2 py-1 w-full min-w-[200px]">
                                <div class="h-3 w-3/4 rounded-full bg-muted animate-pulse"></div>
                                <div class="h-3 w-1/2 rounded-full bg-muted animate-pulse"></div>
...
<script>
    ...
    const [chatTitle, setChatTitle] = pp.state("New Chat");
    const [chats, setChats] = pp.state(<?= json_encode($chats) ?>);
    const [currentChatId, setCurrentChatId] = pp.state(null);

    // Track if we should auto-scroll
    let shouldAutoScroll = true;

    pp.effect(() => {
        document.title = chatTitle + " - PulsePoint AI";
    }, [chatTitle]);
    ...
    function handleScroll(e) {
        ...
        shouldAutoScroll = distanceBottom < 10;
    }
    ...
    function clearChat() {
        ...
        setCurrentChatId(null); // Reset the ID so the backend creates a new row next time
        shouldAutoScroll = true;
        searchParams.delete('chat');
    }

    // NEW: Save the user prompt and full AI response to the database
    async function saveChatTurn(userContent, assistantContent) {
        console.log("Saving chat turn...");
        ...
        try {
            ...
            } = await pp.fetchFunction('saveChatTurn', {
                chatId: currentChatId, // If null, backend creates a new Chat
                title: chatTitle, // Send the current title (e.g., generated one)
            ...
            if (success && chatId) {
                ...
                setChats(prev => {
                    // If chat already exists in list, don't duplicate
    ...
    async function generateChatTitle(firstMessage) {
        try {
            const res = await fetch('/api/chat-title-ollama/', {
                method: 'POST',
                headers: {
    ...
    async function sendMessage() {
        ...
        // --- FORCE SCROLL: When sending, always snap to bottom ---
        shouldAutoScroll = true;
        ...
        // NEW: Buffer to capture the complete response string for saving
        let fullBotResponse = "";
        ...
        try {
            await streamChat(messages, {
                onToken(t) {
                    ...
                    // Accumulate the token into our buffer
                    fullBotResponse += t;
                ...
                onDone() {
                    ...
                    saveChatTurn(text, fullBotResponse);
                },
                onError(err) {
    ...
    async function streamChat(messages, {
        ...
        const res = await fetch("/api/chat-stream-ollama-with-tool/", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
