            ...
            <template pp-for="m in chat">
                ...
                <div class="flex items-start gap-4 {m.role === 'user' ? 'flex-row-reverse' : ''}">
                    ...
                    <div class="space-y-2 w-full">
                        ...
                        {m.role === 'user'
                            ? 'bg-secondary text-secondary-foreground rounded-tr-sm w-fit'
...
<script>
    ...
    // Optional: Store the dynamic title in state if you want to display it
    ...
    async function generateChatTitle(firstMessage) {
        try {
            ...
            if (data.success && data.title) {
                ...
                // Here you would typically save this title to your DB associated with the Chat ID
            }
        } catch (error) {
    ...
    async function sendMessage() {
        ...
        const isFirstMessage = history.length === 0; // Check if this is the start
        ...
        // --- TRIGGER TITLE GENERATION (Parallel) ---
        if (isFirstMessage) {
            // We don't await this because we want the chat to stream immediately
    ...
    async function streamChat(messages, {
        ...
        const res = await fetch("/api/chat-stream-with-tool/", { // Ensure this matches your route file
            method: "POST",
            headers: {
                "Content-Type": "application/json",
